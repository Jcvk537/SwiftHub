...
local function LoadSwiftX()
    ...
    -- Settings Originali + Nuovi
    local AimbotSettings = {
        Enabled = false, Method = "Camera", Part = "Head", Smoothness = 0.5, 
        Prediction = 0.15, FOV = 100, ShowFOV = false, WallCheck = false, TeamCheck = false,
        HitboxExpand = false, HitboxSize = 10
    }
    local VisualSettings = {
        Box = false, Health = false, Tracers = false, Distance = false, 
        Count = false, Accent = Color3.fromRGB(0, 170, 255)
    }
    -- [NEW] Settings Aggiuntivi
    local ExtraSettings = {
        WalkSpeed = 16, JumpPower = 50, InfJump = false, NoClip = false,
        Fullbright = false, Fly = false, FlySpeed = 50, Spinbot = false,
        AutoClicker = false, FOV = 70, InstantPrompt = false
    }
    
    -- Prison Life Settings
    local PrisonLifeSettings = {
        TargetHUD = false,
        SmartTarget = false,
        Invisibility = false,
        GunSwitcher = false,
        ...
        BulletTracer = false,
        TracerTexture = "Default",
        KillSound = false,
        KillSoundID = "9113083740",
        FOVPerformance = 70,
        VehicleSpeed = false,
        VehicleSpeedMult = 2
    }
    ...
    -- Target HUD
    local TargetHUD = Instance.new("ScreenGui", CoreGui)
    TargetHUD.Name = "TargetHUD"
    TargetHUD.Enabled = false
    
    local THFrame = Instance.new("Frame", TargetHUD)
    THFrame.Size = UDim2.fromOffset(250, 120)
    THFrame.Position = UDim2.new(0.5, -125, 0.8, 0)
    THFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
    THFrame.BackgroundTransparency = 0.2
    Instance.new("UICorner", THFrame).CornerRadius = UDim.new(0, 12)
    local THStroke = Instance.new("UIStroke", THFrame)
    THStroke.Color = VisualSettings.Accent
    THStroke.Thickness = 2
    
    local THName = Instance.new("TextLabel", THFrame)
    THName.Size = UDim2.new(1, -20, 0, 30)
    THName.Position = UDim2.new(0, 10, 0, 10)
    THName.BackgroundTransparency = 1
    THName.Text = "Target: None"
    THName.TextColor3 = Color3.new(1, 1, 1)
    THName.Font = Enum.Font.GothamBold
    THName.TextSize = 18
    THName.TextXAlignment = Enum.TextXAlignment.Left
    
    local THHealth = Instance.new("TextLabel", THFrame)
    THHealth.Size = UDim2.new(1, -20, 0, 25)
    THHealth.Position = UDim2.new(0, 10, 0, 45)
    THHealth.BackgroundTransparency = 1
    THHealth.Text = "Health: 0/0"
    THHealth.TextColor3 = Color3.fromRGB(0, 255, 100)
    THHealth.Font = Enum.Font.GothamMedium
    THHealth.TextSize = 16
    THHealth.TextXAlignment = Enum.TextXAlignment.Left
    
    local THDist = Instance.new("TextLabel", THFrame)
    THDist.Size = UDim2.new(1, -20, 0, 25)
    THDist.Position = UDim2.new(0, 10, 0, 75)
    THDist.BackgroundTransparency = 1
    THDist.Text = "Distance: 0m"
    THDist.TextColor3 = Color3.fromRGB(255, 255, 0)
    THDist.Font = Enum.Font.GothamMedium
    THDist.TextSize = 16
    THDist.TextXAlignment = Enum.TextXAlignment.Left
    ...
    function SwiftX:CreateWindow(cfg)
        ...
        UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if (input.Position - tDragStart).Magnitude < 10 then
                    ...
                    if isVisible then
                        Tween(Main, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = UDim2.fromOffset(620, 0), GroupTransparency = 1}).Completed:Connect(function()
                            if Main.GroupTransparency == 1 then Main.Visible = false end
                        end)
                    else
                        Main.Visible = true
                        Tween(Main, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.fromOffset(620, 450), GroupTransparency = 0})
                    end
                end
            end
        end)
        ...
        function Tabs:CreateTab(name)
            ...
            function Elements:CreateDropdown(text, options, callback)
                ...
                Drop.Size = UDim2.new(0.96, 0, 0, 50); Drop.BackgroundColor3 = Color3.fromRGB(18, 18, 22); Instance.new("UICorner", Drop)
                local Title = Instance.new("TextLabel", Drop)
                Title.Text = "   " .. text; Title.Size = UDim2.new(1, 0, 0, 50); Title.TextColor3 = Color3.new(1,1,1); Title.Font = Enum.Font.GothamBold; Title.TextSize = 16; Title.BackgroundTransparency = 1; Title.TextXAlignment = "Left"
                local Arrow = Instance.new("TextLabel", Drop)
                Arrow.Text = "▼"; Arrow.Size = UDim2.new(0, 30, 0, 50); Arrow.Position = UDim2.new(1, -40, 0, 0); Arrow.TextColor3 = Accent; Arrow.Font = Enum.Font.GothamBold; Arrow.TextSize = 14; Arrow.BackgroundTransparency = 1
                local selected = options[1] or "None"
                
                local List = Instance.new("Frame", Drop)
                List.Size = UDim2.new(1, 0, 0, #options * 35); List.Position = UDim2.new(0, 0, 0, 50); List.BackgroundColor3 = Color3.fromRGB(22, 22, 26); List.Visible = false; List.ZIndex = 10; Instance.new("UICorner", List)
                local ListLayout = Instance.new("UIListLayout", List); ListLayout.Padding = UDim.new(0, 2)
                
                for _, opt in pairs(options) do
                    local OptBtn = Instance.new("TextButton", List)
                    OptBtn.Size = UDim2.new(1, 0, 0, 33); OptBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 30); OptBtn.Text = opt; OptBtn.TextColor3 = Color3.new(1,1,1); OptBtn.Font = Enum.Font.GothamMedium; OptBtn.TextSize = 14; OptBtn.ZIndex = 10; Instance.new("UICorner", OptBtn)
                    OptBtn.MouseButton1Click:Connect(function()
                        selected = opt; Title.Text = "   " .. text .. ": " .. selected; List.Visible = false; Arrow.Text = "▼"; callback(selected)
                    end)
                end
                
                Drop.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then List.Visible = not List.Visible; Arrow.Text = List.Visible and "▲" or "▼" end end)
                return Drop
            end
    ...
    Aimbot:CreateDropdown("Aim Part", {"Head", "Torso"}, function(v) AimbotSettings.Part = v end)
    ...
    Aimbot:CreateToggle("Hitbox Expander", function(v) AimbotSettings.HitboxExpand = v end)
    ...
    Aimbot:CreateSlider("Hitbox Size", 1, 50, 10, function(v) AimbotSettings.HitboxSize = v end)
    Aimbot:CreateToggle("Spinbot", function(v) ExtraSettings.Spinbot = v end)
    Aimbot:CreateToggle("Auto Clicker", function(v) ExtraSettings.AutoClicker = v end)
    ...
    Visual:CreateToggle("Fullbright", function(v) ExtraSettings.Fullbright = v end)
    Visual:CreateSlider("Camera FOV", 70, 120, 70, function(v) ExtraSettings.FOV = v end)

    -- [NEW TAB] Movement & Physics
    local Movement = Win:CreateTab("Movement")
    Movement:CreateSlider("WalkSpeed", 16, 250, 16, function(v) ExtraSettings.WalkSpeed = v end)
    Movement:CreateSlider("JumpPower", 50, 500, 50, function(v) ExtraSettings.JumpPower = v end)
    Movement:CreateToggle("Infinite Jump", function(v) ExtraSettings.InfJump = v end)
    Movement:CreateToggle("Noclip", function(v) ExtraSettings.NoClip = v end)
    Movement:CreateToggle("Fly", function(v) ExtraSettings.Fly = v end)
    Movement:CreateSlider("Fly Speed", 10, 300, 50, function(v) ExtraSettings.FlySpeed = v end)

    -- [PRISON LIFE TAB]
    local PrisonLife = Win:CreateTab("Prison Life")
    
    PrisonLife:CreateSection("Combat")
    PrisonLife:CreateToggle("Target HUD", function(v) 
        PrisonLifeSettings.TargetHUD = v 
        TargetHUD.Enabled = v
    end)
    PrisonLife:CreateToggle("Smart Target", function(v) PrisonLifeSettings.SmartTarget = v end)
    PrisonLife:CreateToggle("Auto Shoot", function(v) PrisonLifeSettings.AutoShoot = v end)
    PrisonLife:CreateToggle("Auto Reload", function(v) PrisonLifeSettings.AutoReload = v end)
    PrisonLife:CreateToggle("Gun Switcher", function(v) PrisonLifeSettings.GunSwitcher = v end)
    PrisonLife:CreateDropdown("Bullet Tracer", {"Default", "Laser", "Lightning", "None"}, function(v) 
        PrisonLifeSettings.TracerTexture = v 
        PrisonLifeSettings.BulletTracer = v ~= "None"
    end)
    
    PrisonLife:CreateSection("Player Mods")
    PrisonLife:CreateToggle("Invisibility", function(v) PrisonLifeSettings.Invisibility = v end)
    PrisonLife:CreateToggle("No Jump Cooldown", function(v) PrisonLifeSettings.NoJumpCooldown = v end)
    PrisonLife:CreateToggle("Kill Effect", function(v) PrisonLifeSettings.KillEffect = v end)
    PrisonLife:CreateToggle("Kill Sound", function(v) PrisonLifeSettings.KillSound = v end)
    
    PrisonLife:CreateSection("Vehicle")
    PrisonLife:CreateToggle("Vehicle Speed", function(v) PrisonLifeSettings.VehicleSpeed = v end)
    PrisonLife:CreateSlider("Speed Multiplier", 1, 10, 2, function(v) PrisonLifeSettings.VehicleSpeedMult = v end)
    
    PrisonLife:CreateSection("Performance")
    PrisonLife:CreateSlider("FOV Performance", 60, 120, 70, function(v) 
        PrisonLifeSettings.FOVPerformance = v 
        Camera.FieldOfView = v
    end)
    
    PrisonLife:CreateSection("Server")
    PrisonLife:CreateButton("Server Hop", function()
        local servers = {}
    ...
    Settings:CreateToggle("Instant Interaction", function(v) ExtraSettings.InstantPrompt = v end)
    Settings:CreateButton("Teleport Random Player", function()
        local plrs = Players:GetPlayers()
        local random = plrs[math.random(1, #plrs)]
        if random ~= LocalPlayer and random.Character then
            LocalPlayer.Character:MoveTo(random.Character.HumanoidRootPart.Position)
        end
    end)
    Settings:CreateButton("Rejoin Server", function()
        game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer)
    end)
    Settings:CreateButton("Join Discord", function()
        setclipboard("https://discord.gg/vinal")
    ...
    Settings:CreateButton("Unload Script", function()
        ...
        if TargetHUD then TargetHUD:Destroy() end
        AimbotSettings.Enabled = false
        ExtraSettings.NoClip = false
        AimbotSettings.HitboxExpand = false
    end)

    Win:OpenFirst()

    -- [NEW LOGIC] Movement & Utilities
    RunService.Stepped:Connect(function()
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
            local hum = LocalPlayer.Character.Humanoid
            hum.WalkSpeed = ExtraSettings.WalkSpeed
            hum.JumpPower = ExtraSettings.JumpPower
            
            if ExtraSettings.NoClip then
            ...
            -- No Jump Cooldown
            if PrisonLifeSettings.NoJumpCooldown then
                hum.UseJumpPower = true
                hum.JumpPower = ExtraSettings.JumpPower
            end
        end

        if ExtraSettings.Fullbright then
            Lighting.Brightness = 2; Lighting.ClockTime = 14; Lighting.FogEnd = 100000
        end

        Camera.FieldOfView = ExtraSettings.FOV

        if ExtraSettings.Spinbot and LocalPlayer.Character then
            LocalPlayer.Character.HumanoidRootPart.CFrame *= CFrame.Angles(0, math.rad(45), 0)
        end
        
        -- Invisibility
        if PrisonLifeSettings.Invisibility and LocalPlayer.Character then
            for _, part in pairs(LocalPlayer.Character:GetDescendants()) do
                if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                    part.Transparency = 1
                end
            end
        elseif not PrisonLifeSettings.Invisibility and LocalPlayer.Character then
            for _, part in pairs(LocalPlayer.Character:GetDescendants()) do
                if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
                    if part.Name == "Head" then
                        part.Transparency = 0
                    elseif part.Name:find("Arm") or part.Name:find("Leg") or part.Name:find("Torso") then
                        part.Transparency = 0
                    end
                end
            end
        ...
        -- Vehicle Speed
        if PrisonLifeSettings.VehicleSpeed then
                ...
                if v:IsA("VehicleSeat") and v.Occupant == LocalPlayer.Character.Humanoid then
                    ...
                    if car and car:FindFirstChild("Body") then
                        ...
                        if body:FindFirstChild("Engine") then
                            body.Engine.Velocity = body.Engine.Velocity * PrisonLifeSettings.VehicleSpeedMult
                        end
                    end
                end
            end
    ...
    UserInputService.JumpRequest:Connect(function()
        if ExtraSettings.InfJump and LocalPlayer.Character then
            LocalPlayer.Character.Humanoid:ChangeState("Jumping")
        end
    end)

    game:GetService("ProximityPromptService").PromptButtonHoldBegan:Connect(function(p)
        if ExtraSettings.InstantPrompt then fireproximityprompt(p) end
    end)
    ...
    RunService.RenderStepped:Connect(function()
        if ExtraSettings.Fly and LocalPlayer.Character then
            flyBV.Parent = LocalPlayer.Character.HumanoidRootPart
            flyBV.Velocity = Camera.CFrame.LookVector * ExtraSettings.FlySpeed
        else
            flyBV.Parent = nil
        end
    end)
    ...
    local function UpdateHitboxes()
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if head then
                    if AimbotSettings.HitboxExpand then
                        if not OriginalHitboxes[p] then
                            OriginalHitboxes[p] = {
                                HeadSize = head.Size,
                                HeadTransparency = head.Transparency,
                                HeadCanCollide = head.CanCollide,
                                HeadMassless = head.Massless,
                                HRPSize = hrp and hrp.Size or nil,
                                HRPCanCollide = hrp and hrp.CanCollide or nil
                            }
                        end
                        head.Size = Vector3.new(AimbotSettings.HitboxSize, AimbotSettings.HitboxSize, AimbotSettings.HitboxSize)
                        head.Transparency = 0.9
                        head.CanCollide = false
                        head.Massless = true
                        if hrp then
                            hrp.Size = Vector3.new(AimbotSettings.HitboxSize * 1.5, AimbotSettings.HitboxSize * 1.5, AimbotSettings.HitboxSize * 1.5)
                            hrp.CanCollide = false
                        end
                    else
                        if OriginalHitboxes[p] then
                            local orig = OriginalHitboxes[p]
                            head.Size = orig.HeadSize
                            head.Transparency = orig.HeadTransparency
                            head.CanCollide = orig.HeadCanCollide
                            head.Massless = orig.HeadMassless
                            if hrp and orig.HRPSize then
                                hrp.Size = orig.HRPSize
                                hrp.CanCollide = orig.HRPCanCollide
                            end
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(AimbotSettings.Part) then
                ...
                if vis then
                    ...
                    if PrisonLifeSettings.SmartTarget then
    ...
    local function AutoShoot()
        if not PrisonLifeSettings.AutoShoot then return end
        ...
        if target and target.Character then
            ...
            if tool and tool:FindFirstChild("Fire") then
                ...
                -- Bullet Tracer
                if PrisonLifeSettings.BulletTracer then
                    local tracer = Instance.new("Part")
                    tracer.Anchored = true
                    tracer.CanCollide = false
                    tracer.Material = Enum.Material.Neon
                    tracer.Size = Vector3.new(0.1, 0.1, (Camera.CFrame.Position - targetPos).Magnitude)
                    tracer.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos) * CFrame.new(0, 0, -tracer.Size.Z/2)
                    
                    if PrisonLifeSettings.TracerTexture == "Laser" then
                        tracer.Color = Color3.fromRGB(255, 0, 0)
                        tracer.Size = Vector3.new(0.2, 0.2, tracer.Size.Z)
                    elseif PrisonLifeSettings.TracerTexture == "Lightning" then
                        tracer.Color = Color3.fromRGB(0, 255, 255)
                    else
                        tracer.Color = VisualSettings.Accent
                    end
    ...
    local function UpdateTargetHUD()
        if not PrisonLifeSettings.TargetHUD then return end
        ...
        if target and target.Character then
            ...
            if hum and hrp then
                THName.Text = "Target: " .. target.DisplayName
                THHealth.Text = "Health: " .. math.floor(hum.Health) .. "/" .. math.floor(hum.MaxHealth)
                local dist = math.floor((Camera.CFrame.Position - hrp.Position).Magnitude)
                THDist.Text = "Distance: " .. dist .. "m"
                ...
                elseif hpPercent > 0.3 then
                    THHealth.TextColor3 = Color3.fromRGB(255, 255, 0)
                else
                    THHealth.TextColor3 = Color3.fromRGB(255, 0, 0)
                end
            end
        else
            THName.Text = "Target: None"
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if hum and hum.Health <= 0 then
                    if PrisonLifeSettings.KillEffect then
                        -- Explosion effect
                    ...
                    if PrisonLifeSettings.KillSound then
                        local sound = Instance.new("Sound")
                        sound.SoundId = "rbxassetid://" .. PrisonLifeSettings.KillSoundID
                        sound.Volume = 1
                        sound.Parent = Camera
                        sound:Play()
    ...
    -- Gun Switcher
    local GunList = {"AK-47", "Revolver", "Shotgun", "M9", "Taser"}
    local CurrentGun = 1
    local function GunSwitcher()
        if not PrisonLifeSettings.GunSwitcher then return end
        ...
        if ammo <= 0 then
            ...
            if CurrentGun > #GunList then CurrentGun = 1 end
            
            game:GetService("ReplicatedStorage").Events.GiveTool:FireServer(GunList[CurrentGun])
            LastShot = tick()
        end
    end
    ...
    local function AutoReload()
        if not PrisonLifeSettings.AutoReload then return end
        
        local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("Ammo") and tool:FindFirstChild("MaxAmmo") then
            if tool.Ammo.Value <= 0 or (tool.Ammo.Value < tool.MaxAmmo.Value and tick() - LastShot > 2) then
                game:GetService("ReplicatedStorage").Events.Reload:FireServer()
            end
        end
    end
